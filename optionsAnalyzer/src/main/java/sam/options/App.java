...
public class App 
{
	...
	final static int[] tDates = {9, 14, 18, 23, 38, 63};
	final static String histDate = "2020-08-17";
    ...
    public static void main( String[] args ) throws Exception
    {
    	...
    	for (int i = 0; i < tckr.length; i++) {
    		...
    		for(int e = 0; e < expiryDates.length; e++) {
    			...
    			for(int j = 0; j < optArray.length(); j++) {
    				...
    				if(opt.getString("option_type").equals("call")) 
    					continue;
    	...
    	for(int i = 0; i < optionList.size(); i++) {
    		...
    		if(optionList.get(i).getzScore() > 4) {
    			continue;
    		}
    		...
    		, optionList.get(i).getzScore(), optionList.get(i).getzScoreProb(), optionList.get(i).getStdErrorD(), optionList.get(i).getECD(), optionList.get(i).getDelta(), optionList.get(i).getExtVal(), optionList.get(i).getExpAnnRet());	
    	}
